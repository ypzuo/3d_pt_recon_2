...
NUM_POINTS = 1024	
NUM_EVAL_POINTS = 1024				# Number of predicted points
...
def get_epoch_loss(val_models):
	...
	for b in xrange(batches):
		...
		runlist = [loss, chamfer_distance_scaled, dists_forward_scaled, dists_backward_scaled, emd]
		L,C,F,B, E = sess.run(runlist, feed_dict={pcl_gt:batch_gt})
		_summary_losses = [L, C, F, B, E]
...
if __name__ == '__main__':
	...
	emd = tf.reduce_mean(get_emd_metrics(pcl_gt_scaled, out_scaled, BATCH_SIZE, NUM_EVAL_POINTS))

	# Define Loss to optimize on
	#loss = (dists_forward_scaled + dists_backward_scaled/2.0)*10000
	loss = chamfer_distance_scaled
	...
	ph_summary = [ph_loss, ph_dists_chamfer, ph_dists_forward, ph_dists_backward, ph_emd]
	...
	with tf.Session(config=config) as sess:
		...
		for i in xrange(start_epoch, max_epoch): 
			...
			val_epoch_chamfer, val_epoch_forward, val_epoch_backward, val_epoch_emd, val_summ = get_epoch_loss(val_models)
